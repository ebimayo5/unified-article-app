...
- 完了内容（2026-09-19 19:00〜20:25 Claude Code / セコム料金記事の画像内金額の是正）: 本文修正後、ユーザーから「画像にも金額が入っている」と指摘を受けて記事画像12枚を全点検。3枚に旧価格が焼き込まれていた（アイキャッチは「○○○円」のダミー表記のため対象外、残り9枚は金額なし）。
  ...
  - **要対応（軽微）**: アップロード手順の試行錯誤中に `secom-neo-kodate-rental-vs-purchase-2026-1.png`（ID **1571**）という重複メディアが1件できた。記事からは参照していないので実害はないが、不要ならメディアライブラリから削除すること（削除は破壊的操作のためユーザー判断に委ねている）。
