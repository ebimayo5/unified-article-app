...
- 完了内容（2026-09-12 Codex / post 1190前半CTA誤リンク）: 前半に重複していたサーキュレーターCTA 2本のうち、表示名「サーキュレーター」でリンク先が遊戯王カード「エア・サーキュレーター」だったものと、販売店の長い商品名を表示していたものを削除し、正しいアイリスオーヤマ PCF-MKM15Nへの自然なテキストCTA 1本へ置換。更新前本文を非表示シート`たくみパパ_post1190商品修正バックアップ`へ保存し、公開状態、更新後本文の完全一致、既存画像、下部Rinker 3商品枠の完全一致を検証した。公開ページから実リンクを開き、PCF-MKM15Nの商品ページに到達することも確認済み。
  - 再発防止: 家電等の検索で遊戯王・デュエルマスターズ・TCG等を除外、主商品検索語に用途語が付いた場合も同一カテゴリの前半CTAを二重生成しない、前半リンク文言に販売店の生タイトルを使わない修正を追加。全43本の`test_*.js`と`git diff --check`に合格。Gitコミット`d494706`をorigin/mainへpushし、Apps Scriptソースへ反映済み。Webアプリ版は348のまま（自動投稿のWebアプリ経路へ反映する際は次回デプロイ更新が必要）。自動投稿の開始・停止・再開、本文再生成、OpenAI生成は行っていない。
