...
- やっていること（2026-09-03更新）: post1190（行69）の本文再生成は完了し、パネル側のシートには新しいstructureMemoに沿った本文が入っている（`uaInspectCirculatorRow1190CurrentBody20260903`で確認: structureMemoLength 993≈想定930、hasBrandComparison:true、bodyLength 13991）。ただし①内部リンク（アイリスオーヤマ記事post1158への言及）がまだ入っていない（hasIrisLink:false）、②本文に画像参照が1件も無い（sheetBodyImageRefs:[]、画像追加ステップ未実行）、③本文中に「4つの原因」的な文言がまだ残っている可能性がある（hasFourCausesFraming:true、要目視確認）。ユーザーが「WP更新」を試みたところ、`uaUpdatePublishedWpFromPanelCore_`の画像保護ガードが正しく発火し「公開中の記事にある画像がパネル本文から3件欠落しています（画像ID 1193, 1197, 1195）」でブロックされた——これはバグではなく安全装置が意図通り働いた結果（新本文に旧「4つの原因」版の手順写真3枚が含まれていないため）。WPへは一切反映されていない（ユーザー確認済み）。次のステップ: ユーザーに①アイリスオーヤマ記事への内部リンクを追記、②画像を通常のパネル操作で追加、してから再度「公開中の記事を更新」を試すよう案内する。診断用一回限り関数`uaInspectCirculatorRow1190CurrentBody20260903`（読み取り専用、wordpress.gs）を追加・push済み。
