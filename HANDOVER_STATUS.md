...
- Codex並行作業: 作業中（2026-09-17 22:10 JST）。ユーザー方針「商品なしが自然な記事は停止せず継続し、理由を記録する」に合わせ、`article.gs`の自動商品導線保証を一般化している。本文再生成・自動投稿の再開・WordPress操作は行わない。テスト、commit、push、既存本番URLへの反映を予定。
