...
- 開始時刻: -
- やっていること: -
- 本番に影響する操作の予定（clasp push/deploy・自動投稿の開始/停止/再開など）: -
- 最終更新: 2026-08-27（Claude Code、このセクションを新設）

## 最終更新
- 更新者: Claude Code（このセクションを直近で更新）
- 日時: 2026-08-27
- 切り替え理由: 「内部リンク」候補シートのデータ消失事故が発生し、原因究明・再発防止の修正・本番反映まで完了したため。

## ⚠️ 2026-08-27 発生: 「内部リンク」シートのデータ消失事故（原因判明・修正済み）
...
- 原因（確定・ユーザーにも確認済み）: `uaUpdateInternalLinksFromSitemaps`（[links.gs](unified_article_app/links.gs)）が、**毎回シートを全クリアしてから、今回のクロールで再取得できた行と「手動保持」列にチェックが入っている行だけを書き戻す**設計になっていた。サイトマップ取得やページ再取得（`uaFetchPageInfo_`、SiteGuard/Wordfenceなどのセキュリティプラグインにブロックされた可能性が高い）がほとんど失敗すると、手動保持以外の全候補が消える。
- **応急対応（ユーザー実施）**: Googleスプレッドシートの変更履歴（ファイル→変更履歴を表示）から実行前の版への復元を案内済み。**復元できたかどうかは本ファイル更新時点で未確認。次のエージェントは必ずスプレッドシートの「内部リンク」シートの現在の件数を確認し、89件前後に戻っているか確認すること。**
- **恒久対応（実施・本番反映済み）**: `uaUpdateInternalLinksFromSitemaps`を全面書き換え。**シートを一括クリアする処理を完全に削除**し、再取得に成功したURLだけを1行ずつ upsert（新設の共有関数 `uaUpsertInternalLinkCandidateRow_`）する方式に変更。サイトマップURL未設定・取得失敗・個別ページ取得失敗のいずれの場合も、既存の行には一切触れない。新規テスト`test_internal_link_sitemap_refresh_safety.js`で「クロール全滅時に既存データが消えない」ことを確認済み。
- 本番デプロイ済み: **`@286`**（"Stop internal-link sitemap refresh from deleting existing candidates"）。
- 教訓: 「シート全体を消してから再構築する」系のバッチ処理は、部分的な取得失敗が起きただけで大量のデータ消失につながる。同種の一括更新処理（サイトマップ系・既存記事移行系など）を今後触るときは、同じパターン（全クリア→成功分だけ書き戻し）になっていないか要注意。
