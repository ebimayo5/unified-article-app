...
- **(1) タグ生成の是正: 実装完了・デプロイ待ち**
  - 調査結果: たくみパパ 記事90本／タグ696個（使用中668）、**85%が1記事専用**、1記事あたり平均9.8個。DRIVE BASE 記事131本／タグ705個、72%が1記事専用。上位タグは健全（ディスプレイオーディオ39、カーナビ38）。タグアーカイブは**両サイトともnoindex済み**（SEO SIMPLE PACK）なのでSEO上の害は小さく、問題は読者体験（記事下のタグ10個の大半が行き止まり）。
  - 原因: prompt.gs がタグを10個固定で生成し、既存タグから選ぶ制約がない。
  - 実装: （wordpress.gs）を、既存タグ優先・1記事最大5個・新規作成は1記事1個までに変更。生成AIの出力に依存しない決定論的な絞り込みで、下書き作成(728)と更新(861)の両経路に効く。 と  に分割し、 は互換のため残置。prompt.gs にも「他の記事にも当てはまる語を優先」「上位数件だけ使われる」を追記。
  - 新規テスト （部分一致を既存扱いしないことも検証）。全55本PASS、クリーン、 をorigin/mainへpush。**clasp push/deploy は未実施**。
- **(2) 記事下のタグ表示を2記事以上に絞る: 未実施（Claude Codeからは実行不可）**
  - 両サイトとも SWELL CHILD が有効。コードスニペット系プラグインは無し。子テーマ functions.php への追記が必要。
  - kurashi-ie の子テーマ functions.php は24行・483文字、開始・波括弧対応0・閉じタグ無し・末尾改行ありで、追記して安全な形であることを確認済み。未パッチ。
  - **ブロック理由**: テーマエディタへの書き込みが自動モード分類器に「Production Deploy」として拒否された（claspと同じ）。スニペット本文はユーザーへ提示済み。
