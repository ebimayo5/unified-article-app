...
- やっていること: -
- 本番に影響する操作の予定: -
- 最終更新: 2026-08-28（Claude Code）— Codexの「商品導線保証」機能をレビューし、見つかった不具合5件（下記コミット参照）を修正のうえコミット・本番デプロイ（`@294`）済み。既存ヨギボー記事の修復はまだ未実施。

## 最終更新
- 更新者: Claude Code（このセクションを直近で更新）
- 日時: 2026-08-27
- 切り替え理由: 「内部リンク」候補シートのデータ消失事故が発生し、原因究明・再発防止の修正・さらに根本原因（サイトマップ経由の取得がセキュリティプラグインに阻まれていた）の修正・本番反映まで完了したため。
