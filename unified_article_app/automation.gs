...
function uaStartAutomaticPostingDaily() {
  ...
  try {
    ...
    if (props.getProperty(UA_AUTOMATION_LAST_STARTED_DATE_PROPERTY) === today) return;
    ...
    uaWriteAutomaticPostingStatus_('開始：' + job.keyword, '');
    uaScheduleAutomaticPostingWorker_(1000);
  } finally {
    lock.releaseLock();
  }
...
function uaEnsureAutomaticPostingSheet_() {
  ...
  sheet.getRange('B3').setDataValidation(SpreadsheetApp.newDataValidation().requireValueInList(['4'], true).build());
  sheet.getRange('B4').setDataValidation(SpreadsheetApp.newDataValidation().requireValueInList(['1'], true).build());
...
function uaTestAutomaticPostingLogic() {
  ...
  return { ok: true, tested: ['1日1記事', '午前4時', '画像あり', '下書き/公開分岐', '再開段階'] };
}
